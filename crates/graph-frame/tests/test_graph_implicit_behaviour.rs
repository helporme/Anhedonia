...
fn node_exec_only_when_res_available() {
    ...
    let mut graph = GarphBuilder::new()
        .with_system(system0)
        .with_system(system1)
        .with_kit(composed!(any_storage: AnyStorage => AnyStorage::new()))
        .build();

    graph.kit_mut().any_storage.insert(0u16);
...
fn correct_sorted_node_exec_order() {
    ...
    graph.kit_mut().any_storage.insert(RwLock::new(Vec::<&'static str>::new()));
    graph.kit_mut().any_storage.insert(A);
    graph.kit_mut().any_storage.insert(B);
    graph.kit_mut().any_storage.insert(C);
    graph.kit_mut().any_storage.insert(D);
    graph.kit_mut().any_storage.insert(E);
    graph.kit_mut().any_storage.insert(F);
    graph.kit_mut().any_storage.insert(I);
    graph.kit_mut().any_storage.insert(O);
